...
class BookDownloader:
    ...
    def fetch_bookcode(self):
        ...
        if response.status_code == 200:  # 요청 성공
            ...
            print(f"북코드 추출 실패. Status code: {response.status_code}")
        return None
    ...
    def fetch_file_list(self):
        ...
        response = requests.post(
            self.file_list_url, headers=self.file_list_headers, data=data)
    ...
    def get_file(self, file_name: Optional[str] = None):
        ...
        if self.bookcode:
            print(f"Fetched bookcode: {self.bookcode}")
