...
class BookDownloader:
    ...
    def download_images(self, image_save_path=None):
        ...
        for idx, img_url in enumerate(image_urls, start=1):
            ...
            if response.status_code == 200:
                with open(save_as, "wb") as f:
                    f.write(response.content)
                print(f"이미지 저장 완료: {save_as}")
            else:
                print(f"다운로드 실패: {img_url}, Status code: {response.status_code}")
    ...
    def fetch_image_list(self):
        ...
        json_url = f"https://ibook.tukorea.ac.kr/Viewer/getBookXML/{self.bookcode}"
